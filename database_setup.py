...
class Student(db.Model, UserMixin):
    ...
    def create_new_list(self): #Create new tool_list and auto fill datetime and return that tool_list
        new_list = Tool_list(owner = self, created_datetime = new_date_time(), stored = 0, approved_status = 0, returned_status = 0)
...
class Tool_list(db.Model):
    ...
    def share(self, student_id):
        new_list = Tool_list(owner=db.session.query(Student).filter_by(student_university_ID=student_id).one(), created_datetime=new_date_time(), shared=1, shared_from_ID=db.session.query(Student).filter_by(id=self.owner_id).one().student_university_ID, approved_status=0, returned_status=0, stored = 0, last_edited_datetime=new_date_time())
